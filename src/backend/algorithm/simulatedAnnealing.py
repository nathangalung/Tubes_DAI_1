...
def simulated_annealing_core(cube, initial_temperature, cooling_rate, min_temperature, 
                             max_no_improvement, reheating_factor, objective_history, energy_temp_history,
                             current_cost):
    N = cube.shape[0]
    ...
    max_iterations = objective_history.shape[0]  # Mengambil ukuran array yang sudah dialokasikan
    ...
    for iteration in range(max_iterations):
        ...
        pos1, pos2 = get_random_positions(N)

        # Melakukan pertukaran
        swap_elements(cube, pos1, pos2)

        # Menghitung biaya baru setelah pertukaran dengan menggunakan utils.utils.calculate_element_cost
    ...
    return best_cost, moves, local_optima_count, iteration + 1  # Mengembalikan jumlah iterasi yang benar-benar digunakan
...
def simulated_annealing(cube, initial_temperature=100.0, cooling_rate=0.9995, min_temperature=0.1, 
    ...
    best_cost, moves, local_optima_count, actual_iterations = simulated_annealing_core(
        cube, initial_temperature, cooling_rate, min_temperature,
        max_no_improvement, reheating_factor, objective_history, energy_temp_history, current_cost
    )
    duration = time.time() - start_time

    # Menampilkan state akhir dan hasil
    print("\nKeadaan Akhir Kubus:")
    utils.print_cube(cube)
    ...
    # Plotting nilai objective function terhadap iterasi (hanya hingga actual_iterations)
    plt.figure(figsize=(10, 5))
    plt.plot(objective_history[:actual_iterations])
    plt.xlabel("Iterasi")
    plt.ylabel("Nilai Objective Function")
    plt.title("Nilai Objective Function per Iterasi")
    plt.show()

    # Plotting Energi x Temperatur terhadap iterasi (hanya hingga actual_iterations)
    plt.figure(figsize=(10, 5))
    plt.plot(energy_temp_history[:actual_iterations])
    plt.xlabel("Iterasi")
